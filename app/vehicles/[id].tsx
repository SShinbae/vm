...
export default function VehicleDetailScreen() {
  ...
  const StatCard = ({ title, value, subtitle, icon, gradient = false }: any) => (
    <View style={[
      styles.statCard,
      ...
      <View style={styles.statCardHeader}>
        <View style={[styles.statIcon, gradient && styles.statIconGradient]}>
          <IconSymbol name={icon} size={isDesktopWeb ? 24 : 20} color={gradient ? '#fff' : colors.tint} />
        </View>
        {isDesktopWeb && subtitle && (
          <View style={styles.statTrend}>
            <IconSymbol name="arrow.up.right" size={12} color={colors.success} />
          </View>
        )}
      </View>
      <View style={styles.statContent}>
        <Text style={[styles.statValue, gradient && styles.statValueLight]}>{value}</Text>
        <Text style={[styles.statTitle, gradient && styles.statTitleLight]}>{title}</Text>
        {subtitle && <Text style={[styles.statSubtitle, gradient && styles.statSubtitleLight]}>{subtitle}</Text>}
      </View>
    </View>
  );
  ...
  const TabButton = ({ label, icon, isActive, onPress }: any) => (
    <TouchableOpacity
      ...
      onPress={onPress}
    >
      <IconSymbol name={icon} size={isDesktopWeb ? 18 : 16} color={isActive ? colors.tint : colors.icon} />
        ...
        <View style={styles.paginationButtons}>
          ...
          >
            <IconSymbol
              name="chevron.right"
              size={isDesktopWeb ? 16 : 14}
              color={currentPage === totalPages ? colors.icon + '50' : colors.tint}
            />
          </TouchableOpacity>
        </View>
      </View>
  ...
  const LogItem = ({ log, icon, isServiceLog = false, showDate = true }: any) => {
    const getLogText = () => {
      if (isServiceLog) {
    ...
    return (
      ...
      >
        ...
        <View style={[styles.logIcon, isDesktopWeb && styles.logIconWeb]}>
          <IconSymbol name={icon} size={isDesktopWeb ? 20 : 16} color={colors.tint} />
        </View>
        <View style={styles.logContent}>
          ...
          {getLogSubtext() && (
            <Text style={[styles.logSubtext, isDesktopWeb && styles.logSubtextWeb]}>
              {getLogSubtext()}
            </Text>
          )}
          {showDate && (
  ...
  return (
    <>
      <Head>
        <title>{vehicle ? `${vehicle.year} ${vehicle.make} ${vehicle.model} - Vehicle Management` : 'Vehicle Management'}</title>
      </Head>
      <SafeAreaView style={styles.container}>
      <ScrollView
            ...
            <View style={styles.heroCard}>
              <View style={styles.vehicleHeader}>
                {vehicle.main_image_url && !imageError ? (
                  <View style={{ position: 'relative' }}>
                    ...
                    <View style={styles.ownershipBadge}>
                      <IconSymbol name={vehicle.is_own_vehicle ? "star.fill" : "person.3.fill"} size={10} color="#fff" />
                  ...
                  <View style={styles.vehicleIcon}>
                    <IconSymbol name="car.fill" size={isDesktopWeb ? 36 : 26} color="white" />
                  </View>
                )}
                ...
                <View style={styles.vehicleActions}>
                  ...
                  >
                    <IconSymbol name="pencil" size={isDesktopWeb ? 18 : 16} color={colors.tint} />
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={styles.actionButton}
                    onPress={handleDelete}
                  >
                    <IconSymbol name="trash" size={isDesktopWeb ? 18 : 16} color="#ff4444" />
                  </TouchableOpacity>
                </View>
              </View>
        ...
        <View style={styles.contentContainer}>
          ...
          {vehicle.is_own_vehicle && (
            <View style={styles.sharingCard}>
              ...
              >
                <View style={styles.sharingLeft}>
                  <View style={styles.sharingIcon}>
                    <IconSymbol name="person.3.fill" size={isDesktopWeb ? 22 : 20} color={colors.tint} />
                  </View>
              ...
              {sharingExpanded && vehicle.sharing_info?.shared_with_groups && vehicle.sharing_info.shared_with_groups.length > 0 && (
                <View style={styles.sharingGroupsList}>
                  {vehicle.sharing_info.shared_with_groups.map((groupName, index) => (
                    <View key={index} style={styles.sharingGroupItem}>
                      ...
                      <Text style={styles.sharingGroupName}>{groupName}</Text>
                    </View>
                  ))}
                </View>
              )}
          ...
          <View style={styles.statsGrid}>
            <StatCard
              ...
              value={(() => {
                ...
                return currentMileage ? `${currentMileage.toLocaleString()}` : 'No data';
              })()}
              subtitle={(() => {
                const currentMileage = vehicle.current_mileage && vehicle.current_mileage > 0
                  ? vehicle.current_mileage
            ...
            <StatCard
              ...
              value={(() => {
                ...
                return vehicle.logs?.counts?.fuel_count !== undefined ? `${vehicle.logs.counts.fuel_count}` : 'N/A';
              })()}
              subtitle={vehicle.logs?.latest_fuel ? `${formatDate(vehicle.logs.latest_fuel.date)}` : 'No records'}
              icon="fuelpump.fill"
            />
            <StatCard
              ...
              value={(() => {
                ...
                return vehicle.logs?.counts?.service_count !== undefined ? `${vehicle.logs.counts.service_count}` : 'N/A';
              })()}
              subtitle={vehicle.logs?.latest_service ? `${formatDate(vehicle.logs.latest_service.date)}` : 'No records'}
            ...
            <StatCard
              ...
              value={(() => {
                ...
                return vehicle.logs?.counts?.mileage_count !== undefined ? `${vehicle.logs.counts.mileage_count}` : 'N/A';
              })()}
              subtitle={vehicle.logs?.latest_mileage ? `${formatDate(vehicle.logs.latest_mileage.date)}` : 'No records'}
          ...
          <View style={styles.logsContainer}>
            <View style={styles.logsHeader}>
              <Text style={styles.logsTitle}>
                Recent {activeTab === 'mileage' ? 'Mileage' : activeTab === 'fuel' ? 'Fuel' : 'Service'} Logs
              </Text>
              <TouchableOpacity
            ...
            {activeTab === 'mileage' && (
              <>
                {vehicle.mileage_logs && vehicle.mileage_logs.length > 0 ? (
                  <>
                    {getPaginatedItems(
                      [...vehicle.mileage_logs].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()),
                      mileageCurrentPage
                    ).map((log, index) => (
                      <LogItem
                        key={index}
                  ...
                  <Text style={styles.noLogsText}>No mileage logs recorded yet</Text>
                )}
              </>
            )}
            ...
            {activeTab === 'fuel' && (
              <>
                {vehicle.fuel_logs && vehicle.fuel_logs.length > 0 ? (
                  <>
                    {getPaginatedItems(
                      [...vehicle.fuel_logs].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()),
                      fuelCurrentPage
                    ).map((log, index) => (
                      <LogItem
                        key={index}
                  ...
                  <Text style={styles.noLogsText}>No fuel logs recorded yet</Text>
                )}
              </>
            )}
            ...
            {activeTab === 'service' && (
              <>
                {vehicle.service_logs && vehicle.service_logs.length > 0 ? (
                  <>
                    {getPaginatedItems(
                      [...vehicle.service_logs].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()),
                      serviceCurrentPage
                    ).map((log, index) => (
                      <LogItem
                        key={index}
                  ...
                  <Text style={styles.noLogsText}>No service logs recorded yet</Text>
                )}
              </>
            )}
          </View>
