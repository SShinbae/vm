...
export default function GroupsScreen() {
  ...
  const handleAcceptInvitation = async (
    ...
    const { data, error } = await GroupInvitationService.acceptInvitation(
      invitation.id,
    );
    if (error) {
      dialog.showError("Error", error);
      ...
      dialog.showSuccess(
        "Success",
        `You have joined "${data?.groupName || invitation.groups?.name || "the group"}"`,
      );
    }
  };
    ...
    dialog.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          style: "destructive",
          onPress: async () => {
            const { error } = await GroupInvitationService.declineInvitation(
              invitation.id,
            );
            if (error) {
              dialog.showError("Error", error);
        ...
        groups.length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyDescription}>
              Create a group to share your vehicle information with family or
              friends.
            </Text>
            <TouchableOpacity
