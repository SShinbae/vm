...
describe('Form', () => {
  ...
  describe('Basic Rendering', () => {
    ...
    it('should render custom submit label', () => {
      render(<Form fields={basicFields} onSubmit={jest.fn()} submitLabel="Save" />);
      expect(screen.getByText('Save')).toBeTruthy();
    });

    it('should render cancel button when onCancel is provided', () => {
      render(<Form fields={basicFields} onSubmit={jest.fn()} onCancel={jest.fn()} />);
  ...
  describe('Field Types', () => {
    ...
    it('should render email input fields', () => {
      const fields: FormField[] = [{ name: 'email', label: 'Email', type: 'email' }];
      render(<Form fields={fields} onSubmit={jest.fn()} />);
  ...
  describe('Form Values', () => {
    it('should display initial values', () => {
      ...
      render(
        <Form fields={basicFields} onSubmit={jest.fn()} initialValues={initialValues} />
      );
  ...
  describe('Form Validation', () => {
    ...
    it('should validate maximum length', () => {
      const fields: FormField[] = [
        {
          ...
          label: 'Username',
          type: 'text',
          validation: { max: 10, message: 'Username must not exceed 10 characters' },
        },
      ];
      const onSubmit = jest.fn();
      render(
      ...
      const submitButton = screen.getByText('Submit');
      fireEvent.press(submitButton);
      expect(screen.getByText('Username must not exceed 10 characters')).toBeTruthy();
      expect(onSubmit).not.toHaveBeenCalled();
    });
      ...
      const fields: FormField[] = [
        {
          ...
          label: 'Phone',
          type: 'text',
          validation: { pattern: /^\d{10}$/, message: 'Phone must be 10 digits' },
        },
      ];
      const onSubmit = jest.fn();
      render(
  ...
  describe('Form Submission', () => {
    it('should call onSubmit with form values when valid', () => {
      ...
      const initialValues = { firstName: 'John', email: 'john@example.com' };
      render(
        <Form fields={basicFields} onSubmit={onSubmit} initialValues={initialValues} />
      );
      const submitButton = screen.getByText('Submit');
      fireEvent.press(submitButton);
    ...
    it('should call onCancel when cancel button is pressed', () => {
      ...
      render(<Form fields={basicFields} onSubmit={jest.fn()} onCancel={onCancel} />);
  ...
  describe('Loading State', () => {
    ...
    it('should disable cancel button when loading', () => {
      render(<Form fields={basicFields} onSubmit={jest.fn()} onCancel={jest.fn()} loading />);
      const cancelButton = screen.getByText('Cancel');
