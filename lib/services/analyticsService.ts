import { FuelLogService, ServiceLogService, MileageLogService } from './loggingService';
...
export class AnalyticsService {
  ...
  static async getAnalytics(period: 'last3months' | 'last6months' | 'lastyear' | 'alltime' = 'last6months'): Promise<AnalyticsData> {
    try {
      ...
      const filteredServiceLogs = serviceLogs.filter(log => new Date(log.date) >= startDate);

      // Calculate analytics
      const monthlyTrends = this.calculateMonthlyTrends(filteredFuelLogs, filteredServiceLogs, period);
      const expenseBreakdown = this.calculateExpenseBreakdown(filteredFuelLogs, filteredServiceLogs);
      const vehicleAnalytics = this.calculateVehicleAnalytics(filteredFuelLogs, filteredServiceLogs);

      // Calculate trends (comparing last month to previous month)
      const trends = this.calculateTrends(monthlyTrends);

      const totalExpenses = filteredFuelLogs.reduce((sum, log) => sum + (log.cost || 0), 0) +
   ...
   */
  private static calculateExpenseBreakdown(fuelLogs: FuelLog[], serviceLogs: ServiceLog[]): ExpenseBreakdown[] {
    const totalFuelCosts = fuelLogs.reduce((sum, log) => sum + (log.cost || 0), 0);
    const totalServiceCosts = serviceLogs.reduce((sum, log) => sum + (log.cost || 0), 0);
   ...
   */
  private static calculateVehicleAnalytics(fuelLogs: FuelLog[], serviceLogs: ServiceLog[]): VehicleAnalytics[] {
    ...
    fuelLogs.forEach(log => {
      if (log.vehicles) {
        ...
        if (!vehicleData.has(vehicleKey)) {
          vehicleData.set(vehicleKey, { name: vehicleName, fuelCosts: 0, serviceCosts: 0 });
        }
    ...
    return Array.from(vehicleData.entries()).map(([vehicleId, data]) => ({
      ...
      totalExpenses: Math.round((data.fuelCosts + data.serviceCosts) * 100) / 100
    }));
  }
   ...
   */
  private static calculateTrends(monthlyTrends: MonthlyTrend[]): { fuelTrend: number; serviceTrend: number } {
    ...
    const fuelTrend = previousMonth.fuel > 0
      ? ((lastMonth.fuel - previousMonth.fuel) / previousMonth.fuel) * 100
      : 0;

    const serviceTrend = previousMonth.service > 0
      ? ((lastMonth.service - previousMonth.service) / previousMonth.service) * 100
      : 0;
   ...
   */
  static formatForLineChart(monthlyTrends: MonthlyTrend[], dataType: 'fuel' | 'service' | 'total'): ChartDataPoint[] {
   ...
   */
  static formatForPieChart(expenseBreakdown: ExpenseBreakdown[]): ChartDataPoint[] {
   ...
   */
  static formatForBarChart(vehicleAnalytics: VehicleAnalytics[]): ChartDataPoint[] {
