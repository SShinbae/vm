...
export class ReceiptStorageService {
   ...
   */
  static async getReceiptImageUrl(receiptPath: string): Promise<ApiResponse<string>> {
    try {
      ...
      return { data: null, error: 'Failed to get receipt image URL', loading: false };
    }
  }
   ...
   */
  static async deleteReceiptImage(receiptPath: string): Promise<ApiResponse<boolean>> {
    try {
      ...
      return { data: null, error: 'Failed to delete receipt image', loading: false };
    }
  }
  ...
  static async listUserReceiptImages(): Promise<ApiResponse<any[]>> {
    try {
      ...
      return { data: null, error: 'Failed to list receipt images', loading: false };
    }
  }
   ...
   */
  static async getStorageStats(): Promise<ApiResponse<{
    totalFiles: number;
    totalSize: number;
    ...
    try {
      ...
      return { data: null, error: 'Failed to get storage statistics', loading: false };
    }
  }
   ...
   */
  static async cleanupOldReceipts(daysOld: number = 365): Promise<ApiResponse<number>> {
    try {
      const { data: { user }, error: userError } = await supabase.auth.getUser();
      ...
      return { data: null, error: 'Failed to cleanup old receipts', loading: false };
    }
  }
}