      ...
      if (groupMemberships && groupMemberships.length > 0) {
        ...
        if (allGroupMembers && allGroupMembers.length > 0) {
          ...
          try {
            ...
            console.log('shared_with_groups column not found, showing all group member vehicles');
          }

          const { data: memberVehicles, error: memberVehiclesError } = await vehiclesQuery;
  ...
  static async createVehicle(vehicle: Omit<VehicleInsert, 'user_id'>): Promise<ApiResponse<Vehicle>> {
    try {
      ...
      try {
        ...
        console.log('shared_with_groups column not found, creating vehicle without sharing field');
      }
  ...
  static async toggleVehicleSharing(vehicleId: string, shared: boolean): Promise<ApiResponse<Vehicle>> {
    try {
      ...
      try {
        ...
        return { data: null, error: 'Sharing feature not available - database needs migration', loading: false };
      }
    } catch (error) {
