...
export class StorageSetupTester {
  ...
  static async testBucketExists(): Promise<ApiResponse<boolean>> {
    try {
      ...
      const bucketExists = data?.some(bucket => bucket.id === 'receipt-images') || false;
  ...
  static async testUploadPermissions(): Promise<ApiResponse<boolean>> {
    try {
      ...
      const testFileContent = 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg==';
      ...
      return {
        data: false,
        error: error instanceof Error ? error.message : 'Failed to test upload permissions',
        loading: false
      };
    }
  }
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('column') && error.message.includes('does not exist')) {
          return { data: false, error: 'OCR columns not found in service_logs table', loading: false };
        }
        return { data: false, error: error.message, loading: false };
      }
      ...
      return {
        data: false,
        error: error instanceof Error ? error.message : 'Failed to test schema update',
        loading: false
      };
    }
  }
  ...
  static async runCompleteTest(): Promise<StorageTestResult> {
    ...
    // Summary
    const passedTests = [result.bucketExists, result.canUpload, result.canRead, result.schemaUpdated]
    ...
    if (passedTests === totalTests) {
      console.log('🎉 All storage setup tests passed! OCR feature is ready to use.');
    } else {
      console.log('⚠️  Some tests failed. Check the setup_database.md file for instructions.');
  ...
  static getSetupInstructions(testResult: StorageTestResult): string {
    ...
    if (!testResult.bucketExists) {
      instructions.push('1. Create the "receipt-images" storage bucket in Supabase dashboard');
    ...
    if (!testResult.canUpload || !testResult.canRead) {
      instructions.push('2. Set up Row Level Security (RLS) policies for storage');
    ...
    if (!testResult.schemaUpdated) {
      ...
      instructions.push('   - Run the ALTER TABLE commands in setup_database.md');
      instructions.push('   - Add: receipt_image_url, ocr_extracted_data, auto_filled columns');
    }
